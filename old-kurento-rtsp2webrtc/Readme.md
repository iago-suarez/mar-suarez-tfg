...
-------------------------
You only need a HTTP server and a url to the kurento server.
To start, be sure that your local Kurento server is running and
copy this repository into the server, for example in apache:

```bash
sudo apt-get install -y apache2
sudo cp -r kurento-rtsp2webrtc/ /var/www/html/
sudo chown -R www-data:www-data /var/www/html/kurento-rtsp2webrtc/
```
